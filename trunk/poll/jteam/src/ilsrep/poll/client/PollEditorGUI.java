...
if (score<1) input=false; else win.alert("Score has to be less then 1. Try again");
}
...
if (Integer.parseInt(correct)<max) input=false; else win.alert("You entered a number outside choice scope. Try again");
}
return correct;
...
poll.setCustomEnabled(win.askYesNo("Allow custom choice?") ?" true" : "false");
int choiceNum=Integer.parseInt(win.askUser("How many options will the poll have?"));
...
Object[] a={"Create new poll session","Edit existing oll session at server","Delete existing poll session from server"};
...
jrb.setActionCommand(i.getId());
                ...
                String pollsessToEditChoice = win.getChoice(group,"Choose pollsession");
...
jrb.setActionCommand(i.getId());
                ...
                String pollsessToDeleteChoice = win.getChoice(group,"Choose pollsession");
...
pollSession.setName(win.askUser("Enter pollsession name"));
 pollSession.setTestMode((win.askYesNo("will this run in testmode?") ? "true"
                    : "false"));
 if (pollSession.getTestMode().equals("true")) {
                pollSession.setMinScore(getScore());
            }
int pollNum=Integer.parseInt(win.askUser("How many polls will the session have?"));
...
TcpCommunicator communicator = new TcpCommunicator();
    ...
    private static Pollsession editPollsession(Pollsession source) {
        ...
        if (win.askYesNo("Change test mode value")) {
            source.setTestMode((win.askYesNo("will this run in testmode?") ? "true"
                    : "false"));
        ...
        if (source.getTestMode().compareTo("true") == 0) {
            ...
            if (source.getTestMode() == null) {
                ...
                win.alert("Current minimum score: "
                        + source.getMinScore());
        ...
        for (Poll poll : source.getPolls()) {
            ...
            if (win.askYesNo("Change")) {
                poll
                        .setCustomEnabled(((win.askYesNo(
                                "Allow custom choice for this poll") ?  "true"
                                : "false")));
            }
            ...
            if (source.getTestMode().compareTo("true") == 0) {
                ...
                System.out.println();
                while (true) {
                    correctChoice = Integer.parseInt(win.askUser("Enter correct choice(1 - "
