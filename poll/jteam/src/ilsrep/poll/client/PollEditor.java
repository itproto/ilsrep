...
public class PollEditor {
	...
	name = consoleInputReader.readLine();
	genXml+=" <poll id=\""+Integer.toString(i)+"\" name=\""+name+"\""; 
	if(testMode.indexOf("true")!=-1) {
	...
	name = consoleInputReader.readLine();
	genXml+=" <description>"+ name+"</description>\n<choices>\n";
int n=1;


yesNoChoice="y";
	while(((correct>=n-1)&&(testMode.indexOf("true")!=-1))||(yesNoChoice.indexOf("y")!=-1)){
	
		System.out.println("enter choice");	 
	name = consoleInputReader.readLine();
	genXml+="<choice id=\""+Integer.toString(n)+"\"  name=\""+name+"\" />\n";
	 if (correct<=n){System.out.print("new choice?");
